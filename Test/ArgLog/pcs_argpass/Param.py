...
# set the parameters with the individual functions
        ...
        for wPrefix, wDict in Children.items():
            ...
            if type(wPrefix) != str:
                raise self.DeclarationError(f"{self.FullPrefix}: Name of child '{wPrefix} is not a string")
            wPrefix = wPrefix.strip()
            if wPrefix == '':
                raise self.DeclarationError(f"{self.FullPrefix}: Name of child could not be blank")
